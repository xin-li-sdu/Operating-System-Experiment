...
OS_TCB* OSRdyQueueOut ()
{
	if(OSRdyTCBQueueNum==0){
		...
		OSRdyTCBQueueNum--;
		OS_TCB *ptcb=OSRdyTCBQueueFront;
...
{
	OSRdyTCBQueueNum=0;
	OSRdyTCBQueueFront=0;
	OSRdyTCBQueueRear=0;
}
#else
void OSRdyQueueIn (OS_TCB *ptcb,INT8U a)
{
	OSRdyTCBQueueNum[a]++;
	ptcb->quantum=OS_SCHED_QUANTUM_MAX;
	ptcb->OSRdyTCBNext=0;
	
	if (OSRdyTCBQueueNum[a]==1){
		OSRdyTCBQueueFront[a]=ptcb;
		ptcb->OSRdyTCBPrev=0;
	}else{
		ptcb->OSRdyTCBPrev=OSRdyTCBQueueRear[a];
		OSRdyTCBQueueRear[a]->OSRdyTCBNext=ptcb;
	}
	OSRdyTCBQueueRear[a]=ptcb;
}
OS_TCB* OSRdyQueueOut (INT8U a)
{
	if(OSRdyTCBQueueNum[a]==0){
		return 0;
	}else{
		OSRdyTCBQueueNum[a]--;
		OS_TCB *ptcb=OSRdyTCBQueueFront[a];
		OSRdyTCBQueueFront[a]=OSRdyTCBQueueFront[a]->OSRdyTCBNext;
		OSRdyTCBQueueFront[a]->OSRdyTCBPrev=0;
		return ptcb;
	}
}
void OS_InitRdyQueue (INT8U a)
{
	OSRdyTCBQueueNum[a]=0;
	OSRdyTCBQueueFront[a]=0;
	OSRdyTCBQueueRear[a]=0;
}
#endif
/*******************************************************************************************************/
...
static  void  OS_SchedNew (void)
{
  ...
  //  OSPrioHighRdy = (INT8U)((y << 3) + OSUnMapTbl[OSRdyTbl[y]]);
	...
	{
		if (OSTCBCur!=0)
			OSRdyQueueIn(OSTCBCur,OSTCBCur->OSTCBY);
		//your code:
		OSPrioHighRdy=OSRdyQueueOut(y)->OSTCBPrio;
	}
#endif
